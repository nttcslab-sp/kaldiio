...
def test_read_arks(fname):
    ...
    ark0 = dict(kaldiio.load_ark(os.path.join(os.path.dirname(__file__), 'arks', 'test.ark')))
...
def test_write_compressed_arks(tmpdir, compression_method):
    ...
    arkc_valid = dict(kaldiio.load_ark(os.path.join(os.path.dirname(__file__),
...
def test_append_mode(tmpdir):
    ...
    kaldiio.save_ark(path.join('a2.ark').strpath, {'a': a}, scp=path.join('b2.scp').strpath, append=True)
    kaldiio.save_ark(path.join('a2.ark').strpath, {'b': b}, scp=path.join('b2.scp').strpath, append=True)
