...
class GlobalHeader(object):
    ...
    def compute(array, compression_method):
        if compression_method == CompressionMethod.kAutomaticMethod:
            if array.shape[0] > 8:
                compression_method = CompressionMethod.kSpeechFeature
            else:
                compression_method = CompressionMethod.kTwoByteAuto

        if compression_method == CompressionMethod.kSpeechFeature:
            matrix_type = 'CM'
        elif compression_method == CompressionMethod.kTwoByteAuto or \
                compression_method == CompressionMethod.kTwoByteSignedInteger:
            matrix_type = 'CM2'
        elif compression_method == CompressionMethod.kOneByteAuto or \
                compression_method == CompressionMethod.kOneByteUnsignedInteger or \
                compression_method == CompressionMethod.kOneByteZeroOne:
            matrix_type = 'CM3'
        else:
            raise ValueError('Unknown compression_method: {}'.format(compression_method))

        if compression_method == CompressionMethod.kSpeechFeature or \
                compression_method == CompressionMethod.kTwoByteAuto or \
                compression_method == CompressionMethod.kOneByteAuto:
            min_value = array.min()
            max_value = array.max()
            if min_value == max_value:
                max_value = min_value + (1. + abs(min_value))
            range_ = max_value - min_value
        elif compression_method == CompressionMethod.kTwoByteSignedInteger:
            min_value = -32768.
            range_ = 65535.
        elif compression_method == CompressionMethod.kOneByteUnsignedInteger:
            min_value = 0.
            range_ = 255.
        elif compression_method == CompressionMethod.kOneByteZeroOne:
            min_value = 0.
            range_ = 1.
        else:
            raise ValueError('Unknown compression_method: {}'.format(compression_method))

        return GlobalHeader(matrix_type, min_value, range_, array.shape[0], array.shape[1])
...
class PerColHeader(object):
    ...
    def read(fd, global_header, endian='<'):
        ...
        return PerColHeader(header_array[:, 0], header_array[:, 1], header_array[:, 2], header_array[:, 3])
    ...
    def compute(array, global_header):
        ...
        if array.shape[0] >= 5:
            srows = np.partition(array, [0, quarter_nr, 3 * quarter_nr, array.shape[0] - 1], axis=0)
            p0 = srows[0]
    ...
    def char_to_float(self, array):
        ...
        return np.where(ma1, p0 + (p25 - p0) * array * (1 / 64.),
