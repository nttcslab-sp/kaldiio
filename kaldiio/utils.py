...
if PY3:
    ...
    def my_popen(cmd, mode="r", buffering=-1):
        ...
        if mode == "r":
            ...
            return _wrap_close(
                io.TextIOWrapper(proc.stdout, encoding=default_encoding), proc
            )
        elif mode == "rb":
            proc = subprocess.Popen(
                cmd, shell=True, stdout=subprocess.PIPE, bufsize=buffering
            ...
            return _wrap_close(
                io.TextIOWrapper(proc.stdin, encoding=default_encoding), proc
            )
        elif mode == "wb":
            proc = subprocess.Popen(
                cmd, shell=True, stdin=subprocess.PIPE, bufsize=buffering
    ...
    def tell(self):
        if not self.seekable():
            if PY3:
                ...
                raise IOError
        return sum(f.tell() - self.init_pos[idx] for idx, f in enumerate(self.fds))
...
def parse_specifier(specifier):
    ...
    if len(sp) != 2:
        if ":" not in specifier:
            raise ValueError(
                "The output file must be specified with "
